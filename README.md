...
| **Vulnerable dependencies** | Live [OSV.dev](https://osv.dev) CVE lookups (PyPI/npm) with offline fallback, resolved npm versions from validated `package-lock.json`/`npm-shrinkwrap.json`, robust semver/range parsing, unpinned versions, non-registry (git/url) sources, and typosquatting of popular Web3 packages |
...
tests/             pytest suite (83 tests, ~88% coverage)
