...
[![CI](https://github.com/arraya20/pharos-address-intelligence/actions/workflows/ci.yml/badge.svg)](https://github.com/arraya20/pharos-address-intelligence/actions/workflows/ci.yml)
...
> **Explorer enrichment:** SocialScan's API requires a credential. Set
> `SOCIALSCAN_API_KEY` to enable contract metadata, sampled activity, and token
> discovery. Without it the tool deliberately runs in RPC-only mode and reports
> partial confidence. Core signals remain available, and tracked token holdings
> are verified against the official
...
- SocialScan is used only for enrichment (first/last seen, protocol names).
