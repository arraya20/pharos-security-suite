...
> uncertainty penalty, and floors the risk score at MODERATE (25). Set
> `SOCIALSCAN_API_KEY` to enable SocialScan contract metadata, sampled activity,
> and token discovery. Without it the tool runs in RPC-only mode; tracked token
...
| --- | --- | --- | --- | --- |
| Atlantic Testnet | `https://atlantic.dplabs-internal.com` | 688689 | PHRS | `api.socialscan.io/pharos-atlantic-testnet/v1/developer/api` |
| Pacific Mainnet | `https://rpc.pharos.xyz` | 1672 | PROS | `api.socialscan.io/pharos-mainnet/v1/developer/api` |
...
- Explorer enrichment runs contract metadata, activity, and token discovery calls
  ...
  confidence.
- `SOCIALSCAN_API_KEY` is sent only as an `x-api-key` header and is never included
  in an explorer request URL.
