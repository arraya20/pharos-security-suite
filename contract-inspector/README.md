...
[![CI](https://github.com/arraya20/pharos-contract-inspector/actions/workflows/ci.yml/badge.svg)](https://github.com/arraya20/pharos-contract-inspector/actions/workflows/ci.yml)
...
| Layer | What |
|-------|------|
| **Proxy Detection** | EIP-1167 minimal, EIP-1967 implementation/beacon, confirmed UUPS, OZ legacy, `implementation()` getter |
...
| --- | --- | --- |
| `MAX_BODY_BYTES` | `65536` | Reject oversized JSON bodies with HTTP 413. |
| `RATE_LIMIT_MAX` | `60` | Requests allowed per client and window. |
...
| `API_KEY` | unset | Required for `/inspect` when `HOST` is non-loopback; send as `Authorization: Bearer ...` or `X-API-Key`. |
