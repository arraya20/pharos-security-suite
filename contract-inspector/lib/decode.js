 ...
 */
export async function readMetadata(rpc, addr, block = "latest") {
  ...
  const [name, symbol, decimals, supply, owner] = await Promise.all([
    rpc.ethCallSafe(addr, "0x06fdde03", block), // name()
    rpc.ethCallSafe(addr, "0x95d89b41", block), // symbol()
    rpc.ethCallSafe(addr, "0x313ce567", block), // decimals()
    rpc.ethCallSafe(addr, "0x18160ddd", block), // totalSupply()
    rpc.ethCallSafe(addr, "0x8da5cb5b", block), // owner()
  ]);
 ...
 */
export async function probeInterfaces(rpc, addr, interfaceIds, block = "latest") {
  const results = [];
