...
// ── 3b. UUPS is confirmed through proxiableUUID() on the implementation ──────
{
  const impl = "0xdddddddddddddddddddddddddddddddddddddddd";
  const rpc = new MockRpc({
    storage: {
      [SLOT_1967_IMPL.toLowerCase()]: padAddr(impl),
    },
    calls: {
      "0x52d1902d": { ok: true, data: SLOT_1967_IMPL },
    },
  });
  const r = await resolveProxy(rpc, "0xproxy", "0x6080");
  assert.equal(r.isProxy, true);
  assert.equal(r.type, "UUPS proxy");
  assert.equal(r.impl.toLowerCase(), impl.toLowerCase());
}

// ── 3c. EIP-1967 beacon slot resolves implementation() on the beacon ─────────
{
  const beacon = "0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee";
  const impl = "0xffffffffffffffffffffffffffffffffffffffff";
  const rpc = new MockRpc({
    storage: {
      [SLOT_1967_BEACON.toLowerCase()]: padAddr(beacon),
    },
    calls: {
      "0x5c60da1b": { ok: true, data: padAddr(impl) },
    },
  });
  const r = await resolveProxy(rpc, "0xproxy", "0x6080");
  assert.equal(r.isProxy, true);
  assert.equal(r.type, "EIP-1967 beacon proxy");
  assert.equal(r.beacon.toLowerCase(), beacon.toLowerCase());
  assert.equal(r.impl.toLowerCase(), impl.toLowerCase());
}
