 ...
 */
export async function resolveProxy(rpc, addr, codeHex, block = "latest") {
  ...
  const slot1967 = addrFromSlot(await rpc.getStorageAt(addr, SLOT_1967_IMPL, block));
  if (slot1967) {
    const admin = addrFromSlot(await rpc.getStorageAt(addr, SLOT_1967_ADMIN, block));
    const uuid = await rpc.ethCallSafe(slot1967, PROXIABLE_UUID_SELECTOR, block);
    if (!uuid.ok && uuid.transient) {
      errors.push({ probe: "proxiableUUID", error: uuid.error || "RPC failure" });
    }
    const isUups =
      uuid.ok &&
  ...
  const beacon = addrFromSlot(await rpc.getStorageAt(addr, SLOT_1967_BEACON, block));
  if (beacon) {
    const result = await rpc.ethCallSafe(beacon, IMPLEMENTATION_SELECTOR, block);
    if (!result.ok && result.transient) {
      errors.push({ probe: "beaconImplementation", error: result.error || "RPC failure" });
    }
  ...
  const slotOz = addrFromSlot(await rpc.getStorageAt(addr, SLOT_OZ_LEGACY, block));
  ...
  const res = await rpc.ethCallSafe(addr, IMPLEMENTATION_SELECTOR, block);
  ...
  if (!res.ok && res.transient) {
    errors.push({ probe: "implementationGetter", error: res.error || "RPC failure" });
  }
